...
class MyWindow:
    def __init__(self, win):
        ...
        # Create shape and calculation dropdowns
    ...
    def calculate(self):
        ...
        result = "Result: "  # Add your calculation logic here
        if selected_shape == 'Circle':
            if selected_calculation == 'Area':
                # Parse the dimensions (e.g., radius) and calculate the area
                res = str(3.14 * float(d1)*float(d1))
                result += res
            elif selected_calculation == 'Perimeter':
                # Parse the dimensions (e.g., radius) and calculate the perimeter
                result += "no Perimeter Calculation"
            elif selected_calculation == 'Curved Area':
                # Parse the dimensions (e.g., radius) and calculate the curved area
                result += "Circle Curved Area Calculation"
        elif selected_shape == 'Rectangle':
            if selected_calculation == 'Area':
                # Parse the dimensions (e.g., length and width) and calculate the area
                res = float(d1) * float(d2)
                result += str(res)
            elif selected_calculation == 'Perimeter':
                # Parse the dimensions (e.g., length and width) and calculate the perimeter
                res = 2 * (float(d1) + float(d2))
                result += str(res)
        elif selected_shape == 'Sphere':
            if selected_calculation == 'Volume':
                # Parse the dimensions (e.g., radius) and calculate the volume
                res = 1.33 * 3.14 * (float(d1) * float(d1) * float(d1))
                result += str(res)
            elif selected_calculation == 'Total Surface Area':
                # Parse the dimensions (e.g., radius) and calculate the total surface area
                res = 4 * 3.14 * (float(d1) * float(d1))
                result += str(res)
        elif selected_shape == 'Cone':
            if selected_calculation == 'Volume':
                # Parse the dimensions (e.g., radius and height) and calculate the volume
                res = 0.3333 * 3.14 * (float(d1) * float(d1)) * float(d2)
                result += str(res)
            elif selected_calculation == 'Total Surface Area':
                # Parse the dimensions (e.g., radius and slant height) and calculate the total surface area
